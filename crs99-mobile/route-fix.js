(() => {
  ...
  function genericProposal(title, days, risk = "") {
    const t = normalize(`${title} ${risk}`);
    ...
    const close = Number(days)
      ? `Consigo entregar a primeira versão em até ${days} ${Number(days) === 1 ? "dia útil" : "dias úteis"} após receber os materiais e regras necessários.`
  ...
  function cardData(card) {
    ...
    let proposal = normalize(preview).includes("sem texto de proposta salvo") ? "" : preview;
    if (!proposal && id) proposal = genericProposal(title, days, risk);
  ...
  function slugify(text = "") {
    return normalize(text).replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 90);
  }
